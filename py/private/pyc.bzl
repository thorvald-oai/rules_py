...
def pyc_wired(ctx):
    """Whether this rule carries PYC_ATTRS/PYC_TOOLCHAINS and can compile."""
    return hasattr(ctx.attr, "_pyc_compiler")
...
def bytecode_compatible(exec_runtime, target_runtime):
    ...
    feature release, so a final exec interpreter is usable iff it shares
    major.minor (+ABI) with the target runtime; prereleases require an exact
    version match. Mirrors the `--expect-version` check in pyc_compile.py.
    Needed because exec-tools resolution gates on major.minor only and
    carries an ungated any-version fallback.
    """
