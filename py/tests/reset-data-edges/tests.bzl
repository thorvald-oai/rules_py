...
def _probe_aspect_impl(target, ctx):
    ...
    modes = []
    if ctx.label.name in ["probe", "first", "second", "_launcher.venv", "parent_terminal", "nested_terminal"]:
        modes.append((
            ctx.label.name,
...
terminal = rule(
    ...
    attrs = {
        ...
        "freethreaded": attr.string(default = "", values = ["", "no", "yes"]),
...
def _reset_data_edges_test_impl(ctx):
    ...
    asserts.equals(
        ...
        sorted(analysistest.target_under_test(env)[_ProbeFilesInfo].modes.to_list()),
    )
