...
py_unittest_test = _py_unittest_test
...
def py_test(name, srcs = [], main = None, **kwargs):
    ...
    # `pytest_main` was removed (issue #868). Fail with a migration pointer
    # instead of the opaque "no such attribute 'pytest_main'" the forwarded
    # kwarg would otherwise raise on the underlying rule.
    if "pytest_main" in kwargs:
        fail(
            "py_test no longer supports `pytest_main`; it was removed in 2.0. " +
            "Use py_pytest_test (always drives pytest) for `{}`, or py_unittest_test ".format(name) +
            "for a stdlib unittest suite. See docs/test-drivers.md.",
        )
