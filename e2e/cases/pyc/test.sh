...
# The launcher `pyc` attr defaults to empty = inherit --//py:pyc, for macro
# and direct rule users alike; an explicit value pins the mode regardless.
...
# Compile actions are always declared ("auto") so any consumer can request
# the outputs, but a source-mode binary must not place .pyc files in its
# runfiles — Bazel then never executes the declared compile actions.
source_actions="$("$BAZEL" aquery --"${RULES}":pyc=source "mnemonic('PyCompile', deps(${RULES}/tests/py-venv-multi-exec:test_pyc_source))" | grep -c '^action ' || true)"
test "$source_actions" -gt 0 || fail "expected auto-declared bytecode compile actions in source mode"
"$BAZEL" build --"${RULES}":pyc=source "${RULES}/tests/py-venv-multi-exec:test_pyc_source"
manifest="bazel-bin/py/tests/py-venv-multi-exec/test_pyc_source.runfiles_manifest"
...
# main_from_genrule_bin lists the genrule (not its output file) in srcs, so
# no compile action is declared for the generated .py; the pyc variants list
# the output file label and compile it.
genrule_actions="$("$BAZEL" aquery "mnemonic('PyCompile', deps(${RULES}/tests/main-from-genrule:main_from_genrule_bin))" | grep -c '^action ' || true)"
...
# `pyc` is a plain launcher attribute, not a configuration transition: the
# sibling venv must resolve to a single configured target under source, pyc,
# and pyc_only launchers alike.
...
# label. Compile actions are auto-declared per configured venv, so expect one
# configured venv and one PyCompile action set per python version: the version
# axis forks, the mode axis must not multiply it.
